...
@app.route('/download/gondi-wordnet.csv')
def download_csv():
    if not SUPABASE_URL or not SUPABASE_ANON_KEY:
        logger.error("Supabase credentials are missing.")
        return "Supabase configuration is incomplete.", 500

    # Build the URL for CSV export from the 'wordnet' table
    export_url = f"{SUPABASE_URL}/rest/v1/gondi_wordnet?select=*"
    ...
    try:
        ...
        return Response(
            res.content,
            mimetype="text/csv",
            headers={"Content-Disposition": "attachment; filename=gondi_wordnet.csv"}
        )
    except requests.RequestException as e:
        logger.error("Error fetching CSV from Supabase: %s", e)
        return "Error downloading CSV", 500
...
def admin():
    ...
    if conn:
        ...
        try:
            ...
            # Get approved users
            cur.execute('SELECT * FROM login_database WHERE approved = TRUE;')
...
def bulk_action():
    ...
    try:
        if action == 'approve':
            ...
            flash(f'Successfully approved {len(word_ids)} entries', 'success')
        elif action == 'reject':
            cur.execute(
                "DELETE FROM gondi_wordnet WHERE id = ANY(%s)",
                (word_ids,)
            )
            flash(f'Successfully rejected {len(word_ids)} entries', 'success')
        elif action == 'pending':
            cur.execute(
                "UPDATE gondi_wordnet SET status = 'pending' WHERE id = ANY(%s)",
                (word_ids,)
            )
            flash(f'{len(word_ids)} entries sent to pending for review', 'success')
